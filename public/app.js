...
if (logoutForm) {
  logoutForm.addEventListener("submit", (event) => {
    ...
    fetch("/oauth/logout", { method: "POST", credentials: "same-origin" })
      .then(() => refreshSessionUI())
      .catch(() => {
        document.getElementById("status").textContent = "Não foi possível sair. Tente novamente.";
      });
  });
}
